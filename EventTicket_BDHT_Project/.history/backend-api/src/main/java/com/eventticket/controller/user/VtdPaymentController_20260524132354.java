...
public class VtdPaymentController {
    ...
    public ResponseEntity<Map<String, Object>> sePayWebhook(
        ...
        if (!paymentService.verifySePaySignature(signature, body)) {
        ...
        System.out.println("📝 Nội dung CK: " + content);

        if (content != null) {
            String upperContent = content.toUpperCase();
            // Tìm "DH" trong nội dung → lấy orderId
            int idx = upperContent.indexOf("DH");
            if (idx >= 0) {
                try {
                    // Lấy phần số sau "DH"
                    String afterDH = content.substring(idx + 2).trim();
                    String orderIdStr = afterDH.replaceAll("[^0-9].*", ""); // lấy số liên tiếp
                    Integer orderId = Integer.parseInt(orderIdStr);

                    String transactionId = (String) body.get("reference_number");
                    paymentService.confirmPaymentByOrderId(orderId, transactionId);
                    System.out.println("✅ Xác nhận thành công - OrderId: " + orderId);

                } catch (NumberFormatException e) {
                    System.out.println("⚠️ Không parse được orderId từ: " + content);
                }
            }
        }
