...
public class VtdPaymentController {
    ...
    public ResponseEntity<Map<String, Object>> sePayWebhook(
        ...
        if (content != null) {
            ...
            System.out.println("📝 idx DH: " + idx);

            if (idx >= 0) {
                try {
                    String afterDH = content.substring(idx + 2).trim();
                    System.out.println("📝 afterDH: " + afterDH);
                    String orderIdStr = afterDH.replaceAll("[^0-9].*", "");
                    System.out.println("📝 orderIdStr: " + orderIdStr);
                    Integer orderId = Integer.parseInt(orderIdStr);
                    String transactionId = (String) body.get("referenceCode");
