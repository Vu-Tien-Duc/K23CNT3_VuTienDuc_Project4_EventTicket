...
async function loadPromotions() {
    ...
    try {
        const promos = await window.apiClient.get('/api/ttb/admin/promotions');
        if (promos) {
...
async function loadFilteredPromotions(keywordOverride = null) {
    ...
    try {
        const filtered = await window.apiClient.get(`/api/ttb/admin/promotions${queryString}`);
        if (filtered) {
...
async function openEditModal(promoId) {
    ...
    try {
        const promo = await window.apiClient.get(`/api/ttb/admin/promotions/${promoId}`);
        if (promo) {
...
async function handleFormSubmit(e) {
    ...
    try {
        if (activePromoId === null) {
            // Thêm mới
            await window.apiClient.post('/api/ttb/admin/promotions/add', payload);
            alert('🎉 Đã tạo mã giảm giá mới thành công!');
        } else {
            // Chỉnh sửa
            await window.apiClient.put(`/api/ttb/admin/promotions/update/${activePromoId}`, payload);
...
async function togglePromoStatusSubmit(promoId) {
    try {
        // Backend trả về JSON (promotion mới). Không cần dùng response nhưng giúp bắt lỗi rõ ràng.
        await window.apiClient.patch(`/api/ttb/admin/promotions/toggle-status/${promoId}`);
        // Giữ nguyên bộ lọc hiện tại nếu có
        await loadFilteredPromotions();
    } catch (err) {
        console.error('Lỗi kích hoạt nhanh mã giảm giá:', err);
        const msg = err?.message || 'Không thể bật/tắt mã giảm giá.';
        alert(`❌ Lỗi bật/tắt mã giảm giá: ${msg}`);
    }
}
