    ...
    const rows = items.length ? items.map(item => {
        ...
        return `
            <tr class="border-b border-gray-100 last:border-b-0">
                <td class="py-3 pr-3">
                    <div class="font-extrabold text-slate-800">${eventName}</div>
                    <div class="text-[10px] font-bold text-slate-400 mt-0.5">${typeName}</div>
                </td>
                <td class="py-3 px-3 text-center font-bold text-slate-600">${quantity}</td>
                <td class="py-3 px-3 text-right font-bold text-slate-600">${unitPrice.toLocaleString('vi-VN')} VND</td>
                <td class="py-3 pl-3 text-right font-black text-brand-orange">${subtotal.toLocaleString('vi-VN')} VND</td>
            </tr>
        `;
    }).join('') : `
        <tr>
            <td colspan="4" class="py-6 text-center text-xs font-bold text-slate-400">Không có chi tiết vé trong đơn hàng này.</td>
        </tr>
    `;

    body.innerHTML = `
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            <div class="bg-slate-50 border border-gray-150 rounded-2xl p-4">
                <div class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Trạng thái</div>
                <div class="inline-block mt-2 text-[10px] font-black uppercase px-3 py-1 rounded-full border ${statusInfo.className}">
                    ${statusInfo.label}
                </div>
            </div>
            <div class="bg-slate-50 border border-gray-150 rounded-2xl p-4">
                <div class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Ngày tạo</div>
                <div class="text-xs font-bold text-slate-800 mt-2">${createdAt}</div>
            </div>
            <div class="bg-slate-50 border border-gray-150 rounded-2xl p-4">
                <div class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Cần thanh toán</div>
                <div class="text-sm font-black text-brand-orange mt-2">${finalAmount.toLocaleString('vi-VN')} VND</div>
            </div>
        </div>

        <div class="border border-gray-150 rounded-2xl overflow-hidden">
            <table class="w-full text-xs">
                <thead class="bg-gray-50 text-slate-400 uppercase tracking-wider text-[10px] font-black">
                    <tr>
                        <th class="py-3 px-4 text-left">Vé / sự kiện</th>
                        <th class="py-3 px-3 text-center">SL</th>
                        <th class="py-3 px-3 text-right">Đơn giá</th>
                        <th class="py-3 px-4 text-right">Tạm tính</th>
                    </tr>
                </thead>
        ...
        <div class="mt-5 bg-gray-50 border border-gray-150 rounded-2xl p-4 flex flex-col gap-2 text-xs font-bold">
            ...
            <div class="flex justify-between text-slate-900 border-t border-gray-200 pt-2 mt-1">
                <span>Thành tiền</span>
                <span class="text-brand-orange font-black">${finalAmount.toLocaleString('vi-VN')} VND</span>
            </div>
        </div>
    `;
}
