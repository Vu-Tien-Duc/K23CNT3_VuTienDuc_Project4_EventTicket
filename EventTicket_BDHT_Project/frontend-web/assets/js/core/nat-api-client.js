...
function resolveAppUrl(path) {
    ...
    const appRootRelative = cleanPath.startsWith('pages/') || cleanPath.startsWith('components/') || cleanPath.startsWith('assets/') || cleanPath === 'nat-index.html';

    if (cleanPath === 'nat-index.html') {
        const base = getFrontendBasePath();
        return `${base}/pages/nat-index.html`;
    }
...
function getCurrentUserStorageKey(user = null) {
    const userObj = parseStoredUser(user || localStorage.getItem('currentUser'));
    if (!userObj) return 'guest';
    return String(userObj.userId || userObj.id || userObj.email || 'guest');
}

function getScopedOrderKey(user = null) {
    return `currentOrderId:${getCurrentUserStorageKey(user)}`;
}
...
function clearCurrentCartSession() {
    localStorage.removeItem(getScopedOrderKey());
    localStorage.removeItem('currentOrderId');
    clearCheckoutStorage();
}

function clearCurrentOrderId() {
    localStorage.removeItem(getScopedOrderKey());
    localStorage.removeItem('currentOrderId');
}
...
window.cartSession = {
    getUserKey: getCurrentUserStorageKey,
    getOrderId() {
        const userKey = getCurrentUserStorageKey();
        const scopedOrderId = localStorage.getItem(getScopedOrderKey());
        if (scopedOrderId) return scopedOrderId;
        return userKey === 'guest' ? localStorage.getItem('currentOrderId') : null;
    },
    setOrderId(orderId) {
        if (!orderId) return;
        localStorage.setItem(getScopedOrderKey(), String(orderId));
        localStorage.setItem('currentOrderId', String(orderId));
    },
    clear: clearCurrentCartSession,
    clearOrder: clearCurrentOrderId,
    clearCheckout: clearCheckoutStorage,
    attachUser(data) {
        return {
            ...data,
            userKey: getCurrentUserStorageKey()
        };
    },
    belongsToCurrentUser(data) {
        if (!data?.userKey) {
            return getCurrentUserStorageKey() === 'guest';
        }
        return data.userKey === getCurrentUserStorageKey();
    }
};
...
window.pageUtils = {
    ...
    async setupAuthMenu() {
        ...
        if (btnLogout) {
            btnLogout.addEventListener('click', (e) => {
                ...
                window.location.href = window.pageUtils.resolveUrl('nat-index.html');
            });
        }
    },
    ...
    async populateHeaderLocationFilter() {
        ...
        select.addEventListener('change', () => {
            ...
            const targetPath = isAllEventsPage ? 'pages/user/nat-all-events.html' : 'pages/nat-index.html';
