...
async function loadUsers() {
    ...
    try {
        const users = await window.apiClient.get('/api/ttb/admin/users');
        if (users) {
            allUsers = users;
...
async function handleSearchInput(e) {
    ...
    try {
        const results = await window.apiClient.get(`/api/ttb/admin/users?keyword=${encodeURIComponent(val)}`);
        if (results) {
...
async function loadFilteredUsers() {
    ...
    try {
        const filtered = await window.apiClient.get(`/api/ttb/admin/users${queryString}`);
        if (filtered) {
...
async function blockUser(id) {
    if (confirm('🔒 Bạn chắc chắn muốn KHÓA tài khoản người dùng này? Người dùng sẽ không thể đăng nhập hoặc thanh toán vé.')) {
        try {
            await window.apiClient.put(`/api/ttb/admin/users/block/${id}`);
...
async function unblockUser(id) {
    try {
        await window.apiClient.put(`/api/ttb/admin/users/unblock/${id}`);
...
async function deleteUser(id) {
    if (confirm('⚠️ CẢNH BÁO: Bạn thực sự muốn XÓA VĨNH VIỄN tài khoản người dùng này khỏi cơ sở dữ liệu? Hành động này không thể hoàn tác!')) {
        try {
            await window.apiClient.delete(`/api/ttb/admin/users/delete/${id}`);
...
async function handleCreateUserSubmit(e) {
    ...
    try {
        await window.apiClient.post('/api/ttb/admin/users/add', payload);
