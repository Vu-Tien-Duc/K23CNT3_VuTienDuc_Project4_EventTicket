...
function bindEvents() {
    ...
    document.getElementById('imageUrlInput')?.addEventListener('input', renderImageUrlPreview);
}
...
function openUploadModal(defaultEventId = '') {
    ...
    document.getElementById('uploadModalHint').innerText = 'Chon mot su kien va nhap duong dan anh.';
    document.getElementById('btnSubmitUpload').innerText = 'Luu anh';
    document.getElementById('replaceImageId').value = '';
    document.getElementById('uploadForm').reset();
    document.getElementById('eventInput').disabled = false;
    document.getElementById('imageUrlInput').required = true;
...
function openReplaceModal(imageId) {
    ...
    document.getElementById('uploadModalHint').innerText = `Anh #${image.imageId} se duoc xoa sau khi luu link anh moi thanh cong.`;
    ...
    document.getElementById('imageUrlInput').value = image.imageUrl || '';
...
function renderImageUrlPreview() {
    ...
    preview.innerHTML = `
        <div class="border border-gray-100 rounded-xl overflow-hidden bg-gray-50 max-w-xs">
            <img src="${escapeHtml(resolveBackendAssetUrl(imageUrl))}" class="w-full aspect-video object-cover" alt="Xem truoc anh" onerror="this.parentElement.classList.add('hidden')">
            <div class="px-3 py-2">
                <p class="text-[11px] text-gray-500 font-semibold truncate" title="${escapeHtml(imageUrl)}">${escapeHtml(imageUrl)}</p>
            </div>
        </div>
    `;
}
...
async function handleUploadSubmit(event) {
    ...
    if (!imageUrl) {
        alert('Vui long nhap duong dan anh.');
        return;
    }
    ...
    try {
        if (replaceImageId) {
            ...
            if (currentCount >= 10) {
                ...
                await uploadImage(eventId, imageUrl);
            } else {
                await uploadImage(eventId, imageUrl);
                await deleteImageRequest(eventId, replaceImageId);
            }
        ...
        alert(replaceImageId ? 'Da thay anh thanh cong!' : 'Da them anh thanh cong!');
        closeUploadModal();
        await refreshImages();
    } catch (error) {
        console.error('Loi luu anh:', error);
        alert(`Khong the luu anh: ${error.message || 'Vui long thu lai.'}`);
    } finally {
...
async function uploadImage(eventId, imageUrl) {
    return window.apiClient.post(`/api/ttb/events/${eventId}/images`, { imageUrl });
}

async function deleteImage(eventId, imageId) {
    if (!confirm('Ban thuc su muon xoa anh nay?')) return;
