...
import com.eventticket.service.admin.TtbAdminPromotionService;
...
@RestController
@RequestMapping("/api/ttb/admin/promotions")
@RequiredArgsConstructor // Dùng chuẩn Lombok giống bên Ticket
public class TtbAdminPromotionController {

    // Khai báo private final thay vì @Autowired (Chuẩn xịn của Spring Boot hiện
    // nay)
    private final TtbAdminPromotionService adminPromotionService;

    // Xem danh sách và Lọc (Giữ nguyên gốc nhưng form gọi service đã đổi)
    // API: GET /api/ttb/admin/promotions
    @GetMapping
    ...
    // Lấy chi tiết 1 mã
    // API: GET /api/ttb/admin/promotions/{id}
    @GetMapping("/{id}")
    ...
    // API: POST /api/ttb/admin/promotions/add
    @PostMapping("/add")
    ...
    // API: PUT /api/ttb/admin/promotions/update/{id}
    ...
    // API: PATCH /api/ttb/admin/promotions/toggle-status/{id}
