...
function loginWithFacebook() {
    if (!isConfigured(SOCIAL_AUTH_CONFIG.facebookAppId)) {
        setLoginMessage('Vui lòng cấu hình Facebook App ID trước khi dùng đăng nhập Facebook.');
        return;
    }
    if (!window.FB) {
    ...
    if (window.location.protocol !== 'https:' && window.location.hostname !== '127.0.0.1' && window.location.hostname !== 'localhost') {
        setLoginMessage('Facebook chỉ cho phép đăng nhập khi trang chạy HTTPS hoặc Localhost. Vui lòng kiểm tra lại.', 'orange');
        return;
    }
