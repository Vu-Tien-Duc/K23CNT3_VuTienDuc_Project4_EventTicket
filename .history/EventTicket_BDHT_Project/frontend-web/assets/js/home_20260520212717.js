...
async function fetchHomePageData() {
    try {
        ...
        // Render Banners (Chỉ render nếu có ảnh trả về từ DB, nếu không thì giữ ảnh mặc định)
        ...
        document.getElementById('mini-slider-track').innerHTML = `
            <div class="w-full flex flex-col items-center justify-center py-16 bg-red-50 rounded-2xl border border-red-100">
                ...
                <p class="text-red-600 font-medium">Đã xảy ra lỗi khi tải dữ liệu sự kiện.</p>
            </div>`;
        ...
        document.getElementById('featured-events-wrapper').innerHTML = `
            <div class="w-full flex flex-col items-center justify-center py-20 bg-red-50 rounded-3xl border border-red-100">
                <i class="fas fa-wifi text-4xl text-red-400 mb-3"></i>
                <p class="text-red-600 font-medium">Không thể kết nối đến máy chủ.</p>
            </div>`;
    }
}
    ...
    events.forEach(event => {
        ...
        <div class="mini-slider-card snap-start bg-white rounded-2xl shadow-soft border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer flex flex-col" onclick="window.location.href='event-detail.html?id=${event.eventId}'">
            <div class="h-44 overflow-hidden relative">
                <img src="${event.bannerImageUrl || 'https://via.placeholder.com/300x200?text=No+Image'}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="${event.title}">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl overflow-hidden shadow-soft border border-gray-100 group cursor-pointer h-full flex flex-col hover:-translate-y-2 hover:shadow-2xl transition-all duration-300" onclick="window.location.href='event-detail.html?id=${topEvent.eventId}'">
                <div class="relative h-64 lg:h-72 overflow-hidden">
                    <img src="${topEvent.bannerImageUrl || 'https://via.placeholder.com/600x400?text=No+Image'}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="${topEvent.title}">
        ...
        html += `
            <div class="flex gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 group cursor-pointer" onclick="window.location.href='event-detail.html?id=${event.eventId}'">
                <div class="w-32 h-32 rounded-xl overflow-hidden flex-shrink-0 relative">
                    <img src="${event.bannerImageUrl || 'https://via.placeholder.com/150x150?text=No+Image'}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="${event.title}">
                </div>
