...
function setupExtendFeatures() {
    ...
    btnForgotPassword.addEventListener('click', async (e) => {
        ...
        if (!email) {
            alert("Vui long nhap Email cua ban vao o 'Email dang nhap' roi bam nut Quen mat khau!");
            return;
        }

        try {
            await window.apiClient.post('/api/vtd/public/auth/forgot-password', { email: email });
            const otp = prompt(`Ma OTP da duoc gui den ${email}.\nVui long nhap ma OTP vao day:`);
            if (!otp) {
                alert('Ban da huy thao tac nhap OTP.');
                return;
            }
            await window.apiClient.post('/api/vtd/public/auth/verify-otp', { email: email, otp: otp });
            const newPassword = prompt('Xac thuc OTP thanh cong!\nVui long nhap mat khau moi:');
            if (!newPassword) {
                alert('Ban da huy thao tac dat lai mat khau.');
                return;
            }
            await window.apiClient.post('/api/vtd/public/auth/reset-password', { email: email, newPassword: newPassword });
            alert('Doi mat khau thanh cong! Bay gio ban co the dang nhap bang mat khau moi.');
        } catch (error) {
            alert('Loi: ' + (error.message || 'Da xay ra su co.'));
        }
    });
}
...
function setupSocialLogin() {
    ...
    if (btnFacebook) {
        btnFacebook.addEventListener('click', (e) => {
            e.preventDefault();
            setLoginMessage('Hệ thống đang phát triển. Vui lòng thử lại sau.', 'orange');
        });
    }
