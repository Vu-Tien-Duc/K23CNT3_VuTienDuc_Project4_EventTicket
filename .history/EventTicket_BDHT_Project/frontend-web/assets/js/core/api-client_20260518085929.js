...
window.pageUtils = {
    ...
    async loadFooter() {
            try {
                const response = await fetch('/EventTicket_BDHT_Project/frontend-web/components/footer.html');
                let footerHTML = await response.text(); // Đổi const thành let để có thể sửa chuỗi
                
                // Xử lý đường dẫn cho Footer giống hệt Header (Nếu Footer của bạn có link)
                footerHTML = footerHTML.replace(/href="index\.html"/g, 'href="../index.html"');
                footerHTML = footerHTML.replace(/href="pages\/user\//g, 'href="../pages/user/');
